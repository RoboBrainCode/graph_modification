...
def _add_media_node_to_graph(node_handle, mediatype, mediapath, feed_id):
...
def _add_node_to_graph(node_handle, node_idx, json_feed):
    u_id = 0
    ...
    if (node_handle[0] != '$'):
        ...
        u_id = _add_media_node_to_graph(
            node_handle, mediatype, mediapath, feed_id)
    return u_id

def _add_edge_to_graph(edge_name, from_node_id, to_node_id, edge_props, feed_id):
    props_str = re.sub("'(\\w+)':", r'\1:', str(edge_props))
    print 'relation','edge_name:',edge_name.upper(),'keywords:',str(edge_props['keywords']),'source_text:',str(edge_props['source_text']),'source_url:',str(edge_props['source_url']),'feed_ids:',str(feed_id),'src=',str(from_node_id),'dst=',str(to_node_id)
...
def add_weaver_queries(json_feed):
    ...
    for i, handle in enumerate(handles):
        handle_graph_ids.append(_add_node_to_graph(handle, i, json_feed))
    ...
    for edge in edges_with_indices:
        ...
        _add_edge_to_graph(
            edge[0], from_node_id, to_node_id, edge_props, feed_id)
